...
class SparkHBaseDataFrameTest extends DataPipelineTestBase {
  ...
  test("should be able to read from hbase based on dataframe partitions") {
    ...
    val accountKeysAndRowKeys = loadDataToHbase()
    ...
    val dataSet: Dataset[AccountPosition] = hbaseRepository.readFromHBase(rowKeysDataFrame)
